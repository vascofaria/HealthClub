...
public class GroupClassTest {
	...
	@DataProvider
	private Object[] computeValidDataForSetBeginHourValid() {
		return new Object[] {8 * 60, 22 * 60, 12 * 60};
	}

	@Test(dataProvider = "computeValidDataForSetBeginHourValid")
	public void testSetBeginHourValid(int beginHour) throws InvalidInvocationException {
		// Arrange
		GroupClass groupClass = new GroupClass(8 * 60, 2, 7, new HealthClub("club"), 1, true);
		// Act
		groupClass.setBeginHour(beginHour);
		// Assert
		assertEquals(groupClass.getBeginHour(), beginHour);
	}

	@DataProvider
	private Object[] computeInvalidDataForSetBeginHourValid() {
		return new Object[] {8 * 60 -1 , 22 * 60 + 1};
	}

	@Test(dataProvider = "computeInvalidDataForSetBeginHourValid")
	public void testSetBeginHourInvalid(int beginHour) throws InvalidInvocationException {
		// Arrange
		GroupClass groupClass = new GroupClass(8 * 60, 2, 7, new HealthClub("club"), 1, true);
		// Act
		assertThrows(InvalidInvocationException.class, () -> {groupClass.setBeginHour(beginHour); });
		// Assert
	}
}
